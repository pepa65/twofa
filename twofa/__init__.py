...
def listcmd(pattern):
	...
	if some:
		...
		click.echo("Expiration in {} seconds".format(expire))
	else:
...
def addcmd(label):
	...
	if secrets.get(label):
		raise click.ClickException("label '{}' already present".format(label))
	while True:
		...
		try:
			...
			raise click.ClickException("invalid secret")
			continue
	if secret != "":
...
def renamecmd(label, new_label):
	...
	store = Store()
	secrets = store.load_secrets()
	if secrets[new_label]:
		click.echo("No: label '{}' already exists".format(new_label))
	else:
		try:
